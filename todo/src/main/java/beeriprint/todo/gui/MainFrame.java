...
import beeriprint.todo.util.ProjectComparator;
...
public class MainFrame extends JFrame {
    ...
    private void setup() throws IOException, ClassNotFoundException, SQLException {
        ...
        try (JdbcController controller = new JdbcController();) {
            ...
            loadProjects(controller, "all", "id");
            enableButtonsAndMenues();
        }
    }
    ...
    private void loadProjects(JdbcController controller, String filter, String sortKey) throws SQLException {
    ...
    private void fillProjectTable() {
        ...
        saveProjectMap();
        try (JdbcController controller = new JdbcController();) {
            loadProjects(controller, filterCombo.getSelectedItem().toString(), sortCombo.getSelectedItem().toString());
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex.toString(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void addProject() {
        try (JdbcController controller = new JdbcController();) {
            ...
            statusLabel.setText("Add project #" + id);
            loadProjects(controller, filterCombo.getSelectedItem().toString(), sortCombo.getSelectedItem().toString());
        } catch (Exception ex) {
            logger.log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex.toString(), "Error", JOptionPane.ERROR_MESSAGE);
    ...
    private void deleteTask() {
        ...
        int retVal = JOptionPane.showConfirmDialog(null, msg, "Confirmation", JOptionPane.YES_NO_OPTION);
        if (retVal == JOptionPane.YES_OPTION) {
            Project project = (Project) projectTable.getModel().getValueAt(projectTable.getSelectedRow(), 0);
